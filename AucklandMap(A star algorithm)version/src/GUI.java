...
public abstract class GUI {
	...
	protected abstract void timeModeOn();
	 ...
	 */
	protected abstract void onLoad(File nodes, File roads, File segments, File polygons, File restri);

	protected abstract void ArtPointAll();
	...
	private static final String RESTRI_FILENAME = "restrictions.tab";
	...
	private void initialise() {
		...
		load.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				File nodes = null, roads = null, segments = null, polygons = null, restri = null;
				...
				if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
					...
					for (File f : files) {
						if (f.getName().equals(NODES_FILENAME)) {
							...
							polygons = f;
						} else if (f.getName().equals(RESTRI_FILENAME)) {
							restri = f;
						}
					}
					...
					if (nodes == null || roads == null || segments == null) {
						...
						onLoad(nodes, roads, segments, polygons, restri);
						redraw();
					}
				}
			}
		...
		JButton art = new JButton("Articulation");
		...
		JButton time = new JButton("Time/Distan");
